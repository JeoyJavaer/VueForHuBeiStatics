...
<form class="layui-form" id="search-form" >
	<div class="layui-form" style="width: 100%; margin-top: 20px;">
		...
		<!-- 
		 ...
		 -->	
			<label style="margin-left: 10px;" class="layui-label">分类</label>
			<div style="width: 100px;" class="layui-inline">
				<select  name="categoryId" style="width: 100%;" lay-verify="required">
					<option value=""></option>
				</select>
			</div>
	...
	<table class="layui-hide" id="gameCategoryTable"
		lay-filter="gameCategoryTable"></table>
	...
	<script type="text/javascript">
		...
		//			,type:'datetime'
			...
			table.render({
				elem : '#gameCategoryTable',
				url : 'gameCategory/getListDayCountPvUv',
	...
	//				dataJson[res_data[i].gameName] = res_data[i].countPV;
					countPVArray.push([res_data[i].countDay+"/"+res_data[i].categoryName, res_data[i].countPV]);
					countUVArray.push([res_data[i].countDay+"/"+res_data[i].categoryName, res_data[i].countUV]);
				}
			...
			function createCirclePvOrigin(res){
				...
				for(var i=0;i<res.data.length;i++){
					var json = {};
					json.name = res.data[i].countDay+" / "+res.data[i].categoryName;
			...
			layer.ready(function(){
				$.ajax({
					url: 'gameCategory/getGameCategory.do',
					dataType: 'json',
					type: 'get',
					success: function (data) {
						...
						//	$('#cpId').append(new Option(item.name, item.id));// 下拉菜单里添加元素
							$("select[name=categoryId]").append(new Option(item.categoryName, item.id));
						});
						form.render("select");
					}
				})
			...
			var active = {
				reload : function() {
					...
					//执行重载
					table.reload('gameCategoryTable', {
						page : {
							curr : 1
						//重新从第 1 页开始
						},
			...
			$("#export-button").click(function () {
				var url ='gameCategory/getListDayCountPvUvExport.do';
